...
def resource_msg_creator(quantity, raid_entity, explosive_needed, gunpowder, metal, sulfur, low_grade, metal_pipes, explosive="Rocket"):
...
{str(metal)} Metal Fragments, {str(sulfur)} Sulfur, {str(low_grade)} Low Grade and {str(metal_pipes)} Metal Pipes """
    return msg


def resources_calculator(explosive_number, gunpowder=rocket_cost["gunpowder"], metal=rocket_cost["metal"], sulfur=rocket_cost["sulfur"], low_grade=rocket_cost["low_grade"], metal_pipes=rocket_cost["metal_pipes"]):
    gunpowder_needed = explosive_number * gunpowder
    metal_needed = explosive_number * metal
    sulfur_needed = explosive_number * sulfur
    low_grade_needed = explosive_number * low_grade
    metal_pipes_needed = explosive_number * metal_pipes
    return gunpowder_needed, metal_needed, sulfur_needed, low_grade_needed, metal_pipes_needed


def user_confirmation(raid_entity, quantity, explosive="Rockets"):
    converted_entity = str(raid_entity) + \
        "s" if quantity > 1 else str(raid_entity)
        ...
        if user_confirmation_input.lower() == "yes":
            if raid_type == "door":
                explosive_needed = entity_quantity * \
                    rocket_needed_doors.get(raid_material)
            elif raid_type == "wall":
                explosive_needed = entity_quantity * \
                    rocket_needed_walls.get(raid_material)
            gunpowder_needed, metal_needed,  sulfur_needed, low_grade_needed, metal_pipes_needed = resources_calculator(
                explosive_needed)

            resources_text = resource_msg_creator(quantity=entity_quantity, raid_entity=raid_entity,
                                                  explosive_needed=explosive_needed, gunpowder=gunpowder_needed, metal=metal_needed, sulfur=sulfur_needed, low_grade=low_grade_needed, metal_pipes=metal_pipes_needed)
